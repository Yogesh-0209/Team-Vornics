...
def main():
    logger.info("Starting comprehensive extraction test")
    
    # Create extractor
    extractor = DirectEventExtractor()
    
    # Extract events
    result = extractor.extract_events(test_text, "comprehensive_test.txt")
    ...
    with open("comprehensive_extraction_result.json", "w") as f:
        json.dump(result, f, indent=2)
    
    logger.info("Test completed. Results saved to comprehensive_extraction_result.json")

    # Verify expected events are extracted
    expected_events = [
        "Arrived",           # 10th August 2025 at 05:30
        "Pilot Boarded",     # at 06:15
        "Tugs Made Fast",    # at 07:00
        "Berthed",           # at 08:00
        "Nor Tendered",      # at 09:00
        "Survey",            # at 10:00
        "Hoses Connected",   # at 11:00
        "Loading Started",   # at 12:00
        "Loading Completed", # at 18:00
        "Hoses Disconnected", # at 18:15
        "Survey",            # at 18:30
        "Pilot Boarded",     # 12th August 2025 at 05:30
        "Tugs Made Fast",    # at 05:45
        "Unberthed",         # at 06:15
        "Departed"           # at 06:45
    ]
    
    # Check which expected events were found
    found_events = [event["eventType"] for event in result["events"]]
